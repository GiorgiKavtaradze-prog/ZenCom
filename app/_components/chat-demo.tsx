...
export function ChatDemo() {
  ...
  return (
    <div className="flex h-[420px] w-full flex-col overflow-hidden rounded-2xl border border-black/5 bg-white text-left shadow-[0_30px_80px_-30px_rgba(10,9,24,0.55)]">
      {/* Widget header */}
      <div className="flex items-center gap-3 bg-gradient-to-br from-brand to-brand-2 px-4 py-3.5 text-white">
      ...
      <div
        ref={scrollRef}
        className="flex-1 space-y-2.5 overflow-y-auto bg-[#fafafb] px-3.5 py-4"
      >
      ...
      <div className="flex items-center gap-2 border-t border-black/5 bg-white px-3 py-2.5">
        <div className="flex h-9 flex-1 items-center rounded-full bg-black/[0.04] px-3.5 text-[13px] text-zinc-400">
          Ask anything…
        </div>
  ...
  if (msg.role === "user") {
    return (
      <motion.div {...anim} className="flex justify-end">
        <div className="max-w-[82%] rounded-2xl rounded-br-md bg-gradient-to-br from-brand to-brand-2 px-3.5 py-2.5 text-[13px] leading-relaxed text-white shadow-sm">
          {msg.text}
        </div>
      </motion.div>
    );
